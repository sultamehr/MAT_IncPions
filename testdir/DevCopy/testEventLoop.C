...
{
   for (auto band : data_error_bands) {
      std::vector<CVUniverse*> data_error_band_universes = band.second;
      for (auto universe : data_error_band_universes) {
...
void testEventLoop() {
  ...
  for(auto& var: vars) var->InitializeDATAHists(data_error_bands);
  ...
  LoopAndFillData(data, data_error_bands, vars, studies, mycuts);
  ...
  //for(auto& var: vars) var->hist->SetDirectory(*outDir);
  outDir->Write();
